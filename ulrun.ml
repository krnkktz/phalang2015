...
let exec d s =
  ...
  Ulambda.reduce @@ Ulambda.compile e


let rec top d = (try (
  let () = print_string "phalang15> " in
  let a = read_line () in
  print_string @@ Ulambda.show @@ exec d a ;
  print_newline ()) with
  | Eval.Error x | Syn.Error x | Lex.Error x
  | Iden.Error x | Builtin.Error x | Ulambda.Error x ->
...
(if d then (fun x -> print_string @@ Ulambda.show x ; print_newline ())
else (fun _ -> ())) @@
  exec d @@ fp ;;
